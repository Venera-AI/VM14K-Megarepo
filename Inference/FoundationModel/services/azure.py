import json
import time
from pathlib import Path
from typing import List, Dict
from openai import AzureOpenAI
from azure.ai.inference import ChatCompletionsClient
from azure.ai.inference.models import SystemMessage, UserMessage
from azure.core.credentials import AzureKeyCredential

from .abstract import BatchInference

class AzureInference(BatchInference):

    def __init__(self, api_key: str, model_name: str, system_prompt: str, response_format, azure_endpoint: str):
        """
        Args:
            response_format: Response format to use. Either None or {"type": "json_object"} or json_schema (recommend using Pydantic).
        """
        assert azure_endpoint is not None, f"azure_endpoint must have value. azure_endpoint={azure_endpoint}"
        super().__init__(api_key, model_name, system_prompt, "azure", response_format)
        self.__azure_endpoint: str = azure_endpoint
        self.azure_deployment = self.model_name
        self.isBatchMode = "gpt" in self.model_name
        self.batch_size = 20

        self.__init_client()

    def __init_client(self) -> None:
        if self.isBatchMode:
            print("Batch mode triggered (openAI)")
            self.__client = AzureOpenAI(
                api_version="2024-10-21",
                azure_endpoint=self.__azure_endpoint,
                api_key=self._api_key,
            )
        else:
            print("NON Batch mode triggered (deepseek-r1)")
            self.__client = ChatCompletionsClient(
                endpoint=self.__azure_endpoint,
                credential=AzureKeyCredential(self._api_key),
                api_version="2024-05-01-preview"
            )

    def prepareData(self, data: List[str|Dict]):
        """Prepare data following the format of the model.
        Args:
            data: List of data to prepare.
        Note:
            JsonSchema = {
                "type": "json_schema",
                "json_schema": {
                    "name": "schema-name",
                    "schema": Pydantic_Object.model_json_schema()
                }
            }
        """
        output = []
        for i, datum in enumerate(data):
            if isinstance(datum, Dict):
                i = datum.get('id', i)
                datum = datum.get('content', datum)
            
            azure_input = {
                "custom_id": f"task_{i}",
                "method": "POST",
                "url": "/chat/completions",
                "body": {
                    "model": self.azure_deployment, # deployment = "gpt-4o-globalBatch",
                    "messages": [
                        {"role": "system", "content": self.system_prompt},
                        {"role": "user", "content": datum}
                    ]
                }
            }
            
            # https://platform.openai.com/docs/guides/structured-outputs?api-mode=responses#json-mode
            if self.response_format is not None:
                azure_input["body"]["response_format"] = self.response_format

            output.append(azure_input)
        self.formatted_data = output

    def upload_file_to_server(self) -> None:
        if self.isBatchMode:
            response = self.__client.files.create(file=open(self.input_dir / self.batch_input_file, "rb"), purpose="batch")
            response = json.loads(response.model_dump_json())
            self.file_id = response['id']

            BatchInference.writeJSONL(response, self.output_dir / f"batch_result_upload_{self.uuid}.json")
            print(f"Uploaded file {self.batch_input_file} to Azure. File_id: {self.file_id}")
        else:
            print(f"No need to upload_file_to_server")

    def batch_to_datum(self, id, status_code, batch_id, batch_out):
        outputs = []
        for idx, out in zip(batch_id, batch_out):
            output = {
                "id": f"{id}",
                "custom_id": f"{idx}",
                "response": {
                    "status_code": status_code,
                    "body": {
                        "model": self.model_name,
                        "choices": [
                            {
                                "message": {
                                    "content": out
                                },
                            },
                        ]
                    }
                }
            }
            outputs.append(output)
        return outputs
    
    def create_batch_job(self) -> None:
        if self.isBatchMode:
            assert getattr(self, 'file_id', None) is not None, "'file_id' is required. Please upload_file_to_server."
            response = self.__client.batches.create(
                input_file_id=self.file_id,
                completion_window="24h",
                endpoint=self.__azure_endpoint # "/chat/completions"
            )
            response = json.loads(response.model_dump_json())
            self.job_id = response['id']

            BatchInference.writeJSONL(response, self.output_dir / f"batch_result_job_{self.uuid}.json")
            print(f"Creating batch job for file {self.file_id} on Azure. Created job_id: {self.job_id}")
        else:
            output = []
            MAX_SUCCESS_FAIL = 3
            MAX_RETRIES = 5
            RETRY_DELAY = 2
            data_size = len(self.formatted_data)
            batches = [
                self.formatted_data[i: i + self.batch_size]
                for i in range(0, data_size, self.batch_size)
            ]
            for i, batch in enumerate(batches):
                print(f"{i}", end='-')
                n_successive_fail=0
                time.sleep(0.1)
                batch_id = [datum['custom_id'] for datum in batch]
                questions = "\n\n".join([datum['body']['messages'][1]['content'] for datum in batch])
                for attempt in range(MAX_RETRIES):
                    response = self.get_llm_responese(prompt=questions)
                    
                    if response.get("success"):
                        try:
                            raw_data = response.get("data", "")
                            if len(raw_data) > 0:
                                try:
                                    json_list = raw_data.split("</think>")[1].strip().split()
                                    assert len(json_list) == len(batch_id), f"must match len: len(json_list) ({len(json_list)}) != len(batch_id) ({len(batch_id)})"
                                    output.extend(self.batch_to_datum(i, 200, batch_id, json_list))
                                    n_successive_fail=0
                                    break
                                except json.JSONDecodeError as e:
                                    raise ValueError(f"Error parsing JSON string: {e}")
                            else:
                                json_list = raw_data
                            
                            if not isinstance(json_list, list):
                                    raise ValueError(f"Expected a list of JSON objects, got: {type(json_list)}")
                        
                        except Exception as e:
                            n_successive_fail += 1
                            print("Error processing data:", e)
                            if n_successive_fail>=MAX_SUCCESS_FAIL:
                                print("Face continuous failure. Continue next batch")
                                output.extend(self.batch_to_datum(i, 400, batch_id, ['']*len(batch_id)))
                                break
                    else:
                        print(f"Attempt {attempt + 1} failed. Retrying...")
                        time.sleep(RETRY_DELAY)
                else:
                    print("Failed to get a successful response after maximum retries.")
                    output.extend(self.batch_to_datum(i, 400, batch_id, ['']*len(batch_id)))
                    n_successive_fail+=1
                    if n_successive_fail>=MAX_SUCCESS_FAIL:
                        print("may be rate limited")
            print()
            self.output_data = output

    def get_llm_responese(self, prompt: str):
        assert not self.isBatchMode, f"Only use for NON batch (single call). self.isBatchMode={self.isBatchMode}"
        try:
            response = self.__client.complete(
                messages=[
                    SystemMessage(content=self.system_prompt),
                    UserMessage(content=prompt)
                ],
                max_tokens=16384,
                model="DeepSeek-R1"
            )
            return {"success": True, "data": response.choices[0].message.content}
        except Exception as e:
            print("error in get_llm_responese", e)
            with open("error.log", "a") as f:
                f.write(f"{str(e)}\n")
            return {"success": False}
    
    def get_batch_job_status(self) -> None:
        if self.isBatchMode:
            assert getattr(self, 'job_id', None) is not None, "'job_id' is required. Please create_batch_job."
            response = self.__client.batches.retrieve(self.job_id)
            response = json.loads(response.model_dump_json())
            self.output_file_id = response.get('output_file_id', None)
            self.job_status = response['status'].lower()
            
            BatchInference.writeJSONL(response, self.output_dir / f"batch_result_status_{self.uuid}.json")
            print(f"Getting batch job status for job {self.job_id} on Azure. Output file_id: {self.output_file_id}") 
        else:
            self.job_status = 'completed'

    def download_batch_file(self) -> None:
        if self.isBatchMode:
            assert getattr(self, 'output_file_id', None) is not None, "'output_file_id' is required. Please wait for the batch job to complete. You can get it from 'get_batch_job_status()'."
            output_file = self.output_dir / self.batch_output_file

            response = self.__client.files.content(self.output_file_id).write_to_file(output_file)
            print(f"Downloading batch file for job {self.job_id} on Azure as {output_file}")
        else:
            output_file = self.output_dir / self.batch_output_file
            BatchInference.writeJSONL(self.output_data, output_file)
            print(f'saved results to {output_file}')